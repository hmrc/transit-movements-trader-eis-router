...
final case class MessageSender(itemId: ItemId, messageCorrelationId: Int) {
  override val toString: String = {
    s"MDTP-${pad(itemId.index, 27)}-${pad(messageCorrelationId, 2)}"
  }
...
object MessageSender {
  ...
  private val pattern = "(?i)MDTP-(\\d+)-(\\d+)".r.anchored
